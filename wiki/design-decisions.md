...
- **Import parsing logic (S-06/B-15, not S-04):** Quick entry (S-04) takes structured input — task search field + hours. There is no free-text parsing in quick entry. The parsing question applies only to the **data import** (S-06): exact rules for extracting ticket refs, task titles, and hours from the Excel log format, and how dates are derived from row position vs. explicit date headers. Write `Import::ParseLogService` from scratch — the original parse scripts have been deleted. Decide the date-derivation rule before writing the spec.
