...
describe Task, type: :model do
  ...
  describe "defaults" do
    it "defaults status to active" do
      expect(Task.new.status).to eq("active")
    end

    it "defaults billable to true" do
      expect(Task.new.billable).to be(true)
    end
  end
  ...
  describe "scopes" do
    ...
    describe ".active" do
      ...
      it "excludes archived tasks" do
        expect(Task.active).not_to include(archived_task)
      end
    end
    ...
    describe ".archived" do
      ...
      it "excludes active tasks" do
        expect(Task.archived).not_to include(active_task)
      end
    end

    describe ".billable" do
      it "includes billable tasks" do
        expect(Task.billable).to include(active_task)
      end

      it "excludes non-billable tasks" do
        expect(Task.billable).not_to include(unbillable)
      end
    end
  end
