...
- **Import parsing logic (S-06/B-15, not S-04):** Quick entry (S-04) takes structured input — task search field + hours. There is no free-text parsing in quick entry. The parsing question applies only to the **data import** (S-06): exact rules for extracting ticket refs, task titles, and hours from the Excel log format, and how dates are derived from row position vs. explicit date headers. Pull from `parse_final.rb` before committing B-15, and decide the date-derivation rule before writing the spec. **⚠ Audit required before use:** `parse_final.rb` was AI-generated (OpenCode/Qwen) and has not been reviewed — rewrite freely.
