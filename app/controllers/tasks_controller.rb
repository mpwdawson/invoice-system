class TasksController < ApplicationController
  ...
  def load_form_data
    ...
    if @task&.project_code_id.present? && active_codes.none? { |pc| pc.id == @task.project_code_id }
      @project_codes = active_codes.to_a + [ @task.project_code ]
    else
      @project_codes = active_codes
    end
  end

  def task_params
    params.require(:task).permit(:customer_id, :project_code_id, :title,
