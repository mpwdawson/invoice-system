...
describe CustomerRate, type: :model do
  ...
  describe ".current_for" do
    ...
    context "when no rates exist" do
      it "returns nil" do
        expect(CustomerRate.current_for(customer, Date.today)).to be_nil
      end
    end
    ...
    context "when rates exist" do
      ...
      it "returns the most recent rate on or before the given date" do
        result = CustomerRate.current_for(customer, Date.new(2026, 6, 1))
        expect(result.rate).to eq(150)
      end

      it "matches on the exact boundary date" do
        result = CustomerRate.current_for(customer, Date.new(2026, 1, 1))
        expect(result.rate).to eq(150)
      end

      it "returns nil when all rates are after the given date" do
        expect(CustomerRate.current_for(customer, Date.new(2024, 12, 31))).to be_nil
      end
